...
class AnalysisPlotMotionTracking(RL_EvalCallback):
    ...
    def on_pre_evaluate_policy(self):
        ...
        if self.compute_metrics:
            ...
            self.fail_steps = np.full(self.env.num_envs, -1, dtype=int)
            self.prev_terminate_state = torch.zeros_like(self.terminate_state).bool()
    ...
    def on_post_eval_env_step(self, actor_state):
        ...
        if self.compute_metrics:
            ...
            # Check for newly terminated environments
            if (self.terminate_state != self.prev_terminate_state).any():
                new_terminated_envs = torch.nonzero(
                    torch.logical_and(self.terminate_state, ~self.prev_terminate_state)
                ).squeeze(-1).cpu().numpy()

                for env_id in new_terminated_envs:
                    if self.fail_steps[env_id] == -1:
                        self.fail_steps[env_id] = self.curr_stpes # Record first failure step

                # Update previous termination state
                self.prev_terminate_state = self.terminate_state.clone()
            
            max_steps_per_env = self.env._motion_lib.get_motion_num_steps()

            active_envs = ~self.terminate_state

            # Determine current maximum step among active environments
            if active_envs.sum() > 0:
                curr_max = max_steps_per_env[active_envs].max().item()
            else:
                curr_max = (self.curr_stpes - 1)
                
            self.curr_stpes += 1

            # If all environments terminate or max steps reached, compute metrics
            if self.curr_stpes >= curr_max or self.terminate_state.sum() == self.env.num_envs:
                ...
                # Compute MPJPE and slice data according to fail_steps
                all_mpjpe = [(all_mpjpe[:s, i].mean().item() if s != -1 else all_mpjpe[:, i].mean().item()) for i, s in enumerate(self.fail_steps)]
                all_body_pos_pred = [(all_body_pos_pred[:s, i] if s != -1 else all_body_pos_pred[:, i]) for i, s in enumerate(self.fail_steps)]
                all_body_pos_gt = [(all_body_pos_gt[:s, i] if s != -1 else all_body_pos_gt[:, i]) for i, s in enumerate(self.fail_steps)]
                all_body_rot_pred = [(all_body_rot_pred[:s, i] if s != -1 else all_body_rot_pred[:, i]) for i, s in enumerate(self.fail_steps)]
                all_body_rot_gt = [(all_body_rot_gt[:s, i] if s != -1 else all_body_rot_gt[:, i]) for i, s in enumerate(self.fail_steps)]
                ...
                # If all unique motions have been processed, compute final metrics
                if (self.env.start_idx + self.env.num_envs >= self.env._motion_lib._num_unique_motions):
                    ...
                    pred_rot_all_succ = [(self.pred_rot_all)[i] for i in succ_idxes]
                    gt_rot_all_succ = [(self.gt_rot_all)[i] for i in succ_idxes]

                    pred_pos_all = self.pred_pos_all
                    gt_pos_all = self.gt_pos_all
                    pred_rot_all = self.pred_rot_all
                    gt_rot_all = self.gt_rot_all
                    ...
                    if self.only_print_second_iter:
                        if self.iter_count == 1:
                            ...
                            results = {
                                ...
                                "metrics_all": metrics_all_print,
                                "metrics_succ": metrics_print
                            }
                            return results
                    else:
                        print(f"Success Rate: {self.success_rate:.10f}")
                ...
                # Reset progress bar and metric trackers for next batch of environments
                self.pbar = tqdm(range(self.env._motion_lib._num_unique_motions // self.env.num_envs))
                self.success_rate = 0
                self.curr_stpes = 0
                self.env.start_compute_metrics()
