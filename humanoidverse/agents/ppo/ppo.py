...
class PPO(BaseAlgo):
    ...
    def learn(self):
        if self.init_at_random_ep_len:
            # current_episode_length_buf = self.env.get_episode_length_buf()
            # _ =self.env.set_episode_length_buf(torch.randint_like(current_episode_length_buf, high=int(self.env.max_episode_length)))
    ...
    def _rollout_step(self, obs_dict):
        with torch.inference_mode():
            for i in range(self.num_steps_per_env):
                ...
                # for keys in actor_state.keys():
                #     print(f"action states {keys}: {actor_state[keys].shape}")
                obs_dict, rewards, dones, infos = self.env.step(actor_state)
                # for keys in obs_dict.keys():
                #     print(f"obs_dict {keys}: {type(obs_dict[keys])}")
                # print("rewards: ",rewards.shape)
                # print("rewards: ",rewards.device)
                # print("dones: ",dones.shape)
                # for keys in infos.keys():
                #     print(f"infos {keys}: {type(infos[keys])}")# critic_obs = privileged_obs if privileged_obs is not None else obs
                # for obs_key in obs_dict.keys():
                #     obs_dict[obs_key] = obs_dict[obs_key].to(self.device)
    ...
    def _post_epoch_logging(self, log_dict, width=80, pad=35):
        ...
        if log_dict['ep_infos']:
            for key in log_dict['ep_infos'][0]:
                ...
                if key.startswith('rew_'):
                    reward_name = key[4:]  # 去掉 'rew_' 前缀
                    reward_breakdown_string += f"""{f'Reward {reward_name}:':>{pad}} {value:.4f}\n"""
                else:
        ...
        # 添加reward分解信息
        if reward_breakdown_string:
            log_string += f"\n{'Reward Breakdown:':>{pad}}\n"
            log_string += reward_breakdown_string
    ...
    def _logging_to_writer(self, log_dict, train_log_dict, env_log_dict):
        ...
        # Logging individual reward components
        if log_dict['ep_infos']:
            for key in log_dict['ep_infos'][0]:
                if key.startswith('rew_'):
                    infotensor = torch.tensor([], device=self.device)
                    ...
                    value = torch.mean(infotensor)
                    reward_name = key[4:]  # 去掉 'rew_' 前缀
                    self.writer.add_scalar(f'Reward/{reward_name}', value, log_dict['it'])
    ...
    def evaluate_policy(self):
        ...
        while True:
            ...
            step += 1
            if "success_rate" in actor_state:
                result = actor_state
                break
    ...
    def _create_eval_callbacks(self):
        print("self.config.eval_callbacks: ",self.config.eval_callbacks)
