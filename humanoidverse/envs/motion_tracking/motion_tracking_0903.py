...
class LeggedRobotMotionTracking(LeggedRobotBase):
    ...
    def _reset_root_states(self, env_ids):
        ...
        if self.custom_origins: # trimesh
            ...
            print("reset target of root vel: ", motion_res['root_ang_vel'][env_ids])
            ...
            print("reset target of root vel: ", motion_res['root_ang_vel'][env_ids])
            # --- 在三个 log 文件里追加 "reset DOF" ---
            log_dir = "log_info"
            os.makedirs(log_dir, exist_ok=True)

            log_files = ["actor_obs.log", "critic_obs.log", "action_torque.log"]
            for lf in log_files:
                log_file = os.path.join(log_dir, lf)
                with open(log_file, "a", encoding="utf-8") as f:
                    f.write("=== reset ROOT state ===\n")
                    f.write(f"root_vel: {self.simulator.robot_root_states[env_ids, 7:10]}\n")
                    f.write(f"root_ang_vel: {self.simulator.robot_root_states[env_ids, 10:13]}\n")
    ...
    def _reset_dofs(self, env_ids):
        ...
        print("reset target dof pos: ",self.simulator.dof_pos[env_ids])
        print("reset target dof vel: ",self.simulator.dof_vel[env_ids])
        # --- 在三个 log 文件里追加 "reset DOF" ---
        log_dir = "log_info"
        os.makedirs(log_dir, exist_ok=True)
        ...
        for lf in log_files:
            ...
            with open(log_file, "a", encoding="utf-8") as f:
                f.write("=== reset DOF ===\n")
                f.write(f"dof_pos: {self.simulator.dof_pos[env_ids]}\n")
                f.write(f"dof_vel: {self.simulator.dof_vel[env_ids]}\n")
