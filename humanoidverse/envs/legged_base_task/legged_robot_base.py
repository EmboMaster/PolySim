...
class LeggedRobotBase(BaseTask):
    ...
    def _physics_step(self):
        ...
        for _ in range(self.config.simulator.config.sim.control_decimation):
            ...
            if self.config.simulator.config.name == 'maniskill':
                self.simulator.simulate_at_each_physics_step(self.actions_after_delay)
            else:
    ...
    def _post_physics_step(self):
        ...
        if len(refresh_env_ids) > 0:
            ...
            #     self.simulator._env.scene._gpu_fetch_all()
            #     self.simulator._env.step(action=torch.zeros(self.num_envs, self.dim_actions, device=self.device, requires_grad=False))
