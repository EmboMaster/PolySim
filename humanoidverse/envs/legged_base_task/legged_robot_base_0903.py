...
class LeggedRobotBase(BaseTask):
    ...
    def _init_buffers(self):
        ...
        # print("self.gravity_vec: ",self.gravity_vec.shape)

        try:
            self.projected_gravity = quat_rotate_inverse(self.base_quat, self.gravity_vec)
        except:
            self.base_quat, self.gravity_vec = self.base_quat.to(self.device), self.gravity_vec.to(self.device)
            self.projected_gravity = quat_rotate_inverse(self.base_quat, self.gravity_vec)
    ...
    def _post_physics_step(self):
        ...
        self._refresh_sim_tensors()
        with open(log_file, "a", encoding="utf-8") as f:
            f.write("\n[base before reset in _post_physics_step]\n")
            f.write(f"base_ang_vel:     {self.base_ang_vel}\n")
            f.write(f"base_ang_vel origin after 111:     {self.simulator.robot_root_states[:, 10:13]}\n")
        # set envs
        refresh_env_ids = self.need_to_refresh_envs.nonzero(as_tuple=False).flatten()
        if len(refresh_env_ids) > 0:
            # print("refresh_env_ids: ",refresh_env_ids)
            #import ipdb; ipdb.set_trace()
        ...
        log_file = os.path.join(log_dir, "actor_obs.log")
        with open(log_file, "a", encoding="utf-8") as f:
            f.write("\n[base after reset in _post_physics_step]\n")
            f.write(f"base_ang_vel:     {self.base_ang_vel}\n")
    ...
    def _pre_compute_observations_callback(self):
        ...
        log_file = os.path.join(log_dir, "actor_obs.log")
        with open(log_file, "a", encoding="utf-8") as f:
            f.write("\n[in _pre_compute_observations_callback]\n")
            f.write(f"origin base_ang_vel pre:     {self.simulator.robot_root_states[:, 10:13]}\n")
            f.write(f"self.base_quat {self.base_quat}")
            f.write(f"base_ang_vel after: {self.base_ang_vel[:]}")
    ...
    def reset_envs_idx(self, env_ids, target_states=None, target_buf=None):
        ...
        with open(log_file, "a", encoding="utf-8") as f:
            f.write("\n[before reset]\n")
            f.write(f" base_ang_vel in base:     {self.simulator.robot_root_states[:, 10:13]}\n")
            # f.write(f"self.base_quat {self.base_quat}")
            # f.write(f"base_ang_vel after: {self.base_ang_vel[:]}")# if target_states is not None, reset to target states
        self._reset_robot_states_callback(env_ids, target_states)
        with open(log_file, "a", encoding="utf-8") as f:
            f.write("\n[after reset]\n")
            f.write(f" base_ang_vel after in base:     {self.simulator.robot_root_states[:, 10:13]}\n")
        # fill extras
        self.extras["episode"] = {}
    ...
    def _post_config_observation_callback(self):
        ...
        for obs_key, obs_config in self.config.obs.obs_dict.items():
            ...
            # 日志文件路径
            log_file = os.path.join(log_dir, f"{obs_key}.log")

            # 收集输出内容
            lines = []
            lines.append(f"\n[Obs拼接结果] {obs_key}")
            lines.append("=" * 120)
            lines.append("{:<20} {:<20} {:<10} {:<60}".format("子项名", "张量shape", "dim", "数据"))
            lines.append("-" * 120)

            for key in obs_keys:
                t = self.obs_buf_dict_raw[obs_key][key]
                data_str = str(t[0].cpu().numpy().tolist())  # 取第一个环境的数据
                lines.append("{:<20} {:<20} {:<10} {:<60}".format(
                    key,
                    str(list(t.shape)),
                    str(t.shape[-1]),
                    data_str
            ...
            # 总拼接结果
            total_data_str = str(self.obs_buf_dict[obs_key][0].cpu().numpy().tolist())
            lines.append("{:<20} {:<20} {:<10} {:<60}".format(
                "总拼接结果",
                str(list(self.obs_buf_dict[obs_key].shape)),
                str(self.obs_buf_dict[obs_key].shape[-1]),
                total_data_str
            ))
            lines.append("=" * 120)
            ...
            with open(log_file, "a", encoding="utf-8") as f:
                f.write("\n".join(lines) + "\n")
            #print("obs_key:",obs_key,self.obs_buf_dict[obs_key])
    ...
    def _compute_torques(self, actions):
        ...
        if control_type=="P":
            # print("actions_scaled.shape:", actions_scaled.shape)
            # print("action:", actions)
            # print("default_dof_pos.shape:", self.default_dof_pos.shape)
            # print("simulator.dof_pos.shape:", self.simulator.dof_pos.shape)
            # print("simulator.dof_vel.shape:", self.simulator.dof_vel.shape)
            # print("p_gains.shape:", self.p_gains.shape)
            # print("d_gains.shape:", self.d_gains.shape)
            # print("kp_scale.shape:", getattr(self._kp_scale, "shape", "scalar"))
            # print("kd_scale.shape:", getattr(self._kd_scale, "shape", "scalar"))
            # print("simulator.dof_pos: ", self.simulator.dof_pos)
            # print("delta: ", self.default_dof_pos - self.simulator.dof_pos)
            ...
            # print("torques 2 :",torques)
        elif control_type=="T":
            torques = actions_scaled
            # print("torques 3 :",torques)
        else:
        ...
        # ----- 保存日志 -----
        log_file = os.path.join(log_dir, "action_torque.log")
        with open(log_file, "a", encoding="utf-8") as f:
            f.write("\n[Action & Torque]\n")
            f.write(f"actions (raw):     {actions.cpu().numpy().tolist()}\n")
            f.write(f"actions_scaled:    {actions_scaled.cpu().numpy().tolist()}\n")
            f.write(f"torques (origin):  {torques.cpu().numpy().tolist()}\n")
            f.write("=" * 120 + "\n")
        print("torques origin: ",torques)
        if self.config.robot.control.clip_torques:
            # print("torques 4: ",torques)
