...
def main(override_config: OmegaConf):
    ...
    if not override_config.eval:
        override_config = OmegaConf.create({k: v for k, v in override_config.items() if k != "algo"})
    ...
    result = algo.evaluate_policy()


    parent_dir = Path(override_config.checkpoint).parent.parent
    basename = os.path.basename(config.checkpoint)
    save_path = f"{parent_dir}/{simulator_type}_{config.checkpoint.split('/')[-1]}.jsonl"
