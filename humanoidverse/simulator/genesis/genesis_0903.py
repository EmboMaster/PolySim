...
class Genesis(BaseSimulator):
    ...
    def prepare_sim(self):
        ...
        self.base_lin_vel = self.robot.get_vel().to(self.device)
        self.base_ang_vel = self.robot.get_ang().to(self.device)
        ...
        self.dof_pos = self.robot.get_dofs_position(self.dof_ids).to(self.device)
        self.dof_vel = self.robot.get_dofs_velocity(self.dof_ids).to(self.device)
        ...
        print("contact_forces_shape: ", self.contact_forces.shape)
        print("--------------------------")
    ...
    def refresh_sim_tensors(self):
        ...
        self._rigid_body_pos = self.robot.get_links_pos()[:, self.link_mapping_genesis_to_humanoidverse_idx].to(self.device)
        self._rigid_body_rot = self.robot.get_links_quat()[:, self.link_mapping_genesis_to_humanoidverse_idx].to(self.device)  # (num_envs, 4) isaacsim uses wxyz, we keep xyzw for consistency
        self._rigid_body_rot = self._rigid_body_rot[..., [1, 2, 3, 0]]
        self._rigid_body_vel = self.robot.get_links_vel()[:, self.link_mapping_genesis_to_humanoidverse_idx].to(self.device)
        self._rigid_body_ang_vel = self.robot.get_links_ang()[:, self.link_mapping_genesis_to_humanoidverse_idx].to(self.device)
    ...
    def set_dof_state_tensor(self, set_env_ids, dof_states):
        ...
        # reset dofs
        print("reset dof_pos: ",dof_pos)
        print("rset dof_vel: ",dof_vel)
